...
| アバター本体 | 一部管理対象 | **構造（オブジェクト/コンポーネントの追加削除、ボーン姿勢）はスコープ外。ただし Track 済みサブツリー配下の既存コンポーネントの既存フィールド値（BlendShape 値・マテリアル参照を含む）はホワイトリストで管理**（1.4 参照） |
...
├── Body                              ← ユーザー領域（構造は本ツールが一切触らない。1.4 参照）
├── Armature                          ← ユーザー領域（同上）
│   └── Hips/Spine/...
...
- **アバター本体（Body / Armature 等）の構造は完全にスコープ外。** ボーンの追加削除・コンポーネントの追加削除・ボーンの姿勢は本ツールの管理対象外であり、復元によって巻き戻ることもない。**ただし** 1.4 の仕組みで明示的に Track したサブツリーに限り、既存コンポーネントの既存フィールド値（BlendShape 値・マテリアル参照を含む）だけは例外的に記録・復元される。
...
セクション1.3で「アバター本体はスコープ外」としたが、**構造（オブジェクト/コンポーネントの追加削除、ボーンの姿勢）を変えない限り**、Track 済みサブツリー配下の既存コンポーネントの既存フィールド値は例外的に管理対象とする。コンテナ方式（破棄→再生成）とは扱いが異なるため、別領域として設計する。

対象は自動選択ではなく、`AvatarVcsTrackedReference` マーカー（`GameObject > AvatarVCS > Track Properties Here`）でユーザーがサブツリー単位で opt-in する。マークしたオブジェクト自身とその子孫すべてが対象になるが、`[AvatarVCS]` 配下（コンテナ管理領域）は常に除外される（コンテナの破棄→再生成と二重管理になるため）。
...
| **Track 済みサブツリー配下の任意コンポーネントの既存フィールド値**（`[AvatarVCS]` 配下を除く） | 各 Component の `SerializedObject` | 不要（値の上書きのみ、コンテナと同じ `ComponentCapturer`/`ComponentApplier` を再利用） | ✅ |
| テクスチャの差し替え | Material 内のテクスチャスロット | 必要 | ❌ v1.x |

BlendShape とマテリアル参照は、上記「任意コンポーネントのフィールド値」の中でも**名前 / GUID ベースで解決する特別扱い**（1.4.2 参照）。汎用フィールドキャプチャは SerializedProperty のパス（配列インデックス含む）で解決するため、Mesh のブレンドシェイプ構成やマテリアルスロット数が変わると壊れやすい。この2つだけは更新耐性のために別ルートで扱い、汎用キャプチャ側では二重管理を避けるため明示的に除外する。
  ...
  "avatarReferences": [
    {
      ...
      "components": [ /* ContainerSnapshot.components と同じ ComponentState 配列。
                          path は Track したオブジェクト（例では "Body"）からの相対パス */ ]
    }
  ]
}
```

`avatarReferences[].components` は `ContainerSnapshot.components` と全く同じ形（`ComponentCapturer`/`ComponentApplier` を共用）で、Track したオブジェクトとその子孫すべてのコンポーネント（Transform 自身・マーカーコンポーネント・`[AvatarVCS]` 配下は除く）を対象にする。`blendShapes`/`materials` に該当するフィールド（`m_BlendShapeWeights`/`m_Materials`）はここでは重複して記録しない（1.4.1 参照）。
...
| `avatarReferences`（アバター本体） | **上書きのみ**（JSON にないオブジェクト/コンポーネントは一切触らない） |

理由: アバター本体はユーザー領域であり、ツールが把握していない値を勝手にリセットすると事故につながるため。BlendShape は名前ベースで、`components` の各コンポーネントはパスベースで解決し、対象が存在しなければ警告してスキップする（コンポーネントを新規作成することはない）。
