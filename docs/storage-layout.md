...
> `AvatarVcsRoot` コンポーネントが持っていて、`ProjectSettings/` 側にはどのアバターの
> ものかを示す情報がありません。つまり**このコンポーネントを失うと履歴も辿れなくなります**。
> シーンから `[AvatarVCS]` を消す、Missing になったスクリプトを掃除する、といった操作は
...
## 壊れたファイル・新しすぎるファイルの扱い

このディレクトリの JSON は、いずれも「読めなかったら空として扱う」ではなく次の2つのルールで
守られています（`StoredSchema` / `StoredJson`）。

1. **読めないファイルの上には書かない。** クラッシュで切れた・手で壊した `index.json` を
   そのまま上書きすると、コミット一覧が消えて元に戻せません。書き込み前に
   `index.json.corrupt-YYYYMMDD-HHMMSS` へ退避してから新しいファイルを作ります
   （退避したファイルは自動では消しません）。
2. **自分より新しい schemaVersion のファイルは書き換えない。** `JsonUtility` は知らないキーを
   落とすので、新しい版が書いたファイルを古い版が読んで保存し直すと、その差分が消えます。
   該当した場合は警告を出し、保存操作自体を中止します（パッケージを更新してください）。

`index.json` と `config.json` は**コミットファイルから再構築できます**。前者は全フィールドが
コミットの写しであり、後者もブランチの HEAD ＝そのブランチの最新コミット、というだけだからです。
そのため読めなくなっても履歴は失われません。再構築できないのは
「作っただけで1度もコミットしていないブランチ」と `guid-remapping.json`（ユーザーが答えた
対応表で、どこにも控えがない）の2つです。
...
`materialSettings` を含む構成を checkout すると、**記録された設定は元マテリアルにそのまま書き込まれます**。
BlendShape 値やコンポーネントのフィールド値と同じ扱いです。

複製を作るのは次の2つの場合だけで、しかも**1マテリアルにつき常に1個**（コミットごとには作りません）:

1. **アバターの外にあるレンダラーも同じマテリアルを使っている場合** — 他のアバターが勝手に変わるのを
   避けるため、このアバター用の複製に切り替えます。Console に「どのオブジェクトが使っているか」を出します。
   `Tools > AvatarVCS > Overwrite Shared Materials` を ON にすると、この場合も元マテリアルを書き換えます。
2. **元マテリアルが `Packages/` 配下（書き込み不可）の場合** — `Assets/AvatarVCS_Generated/` に複製します。

複製には「どのマテリアルから作ったか」が `.meta` の userData に記録され、コミット時はそれを辿って
**元マテリアル**を `sourceMaterialGuid` として記録します。これをしないと、checkout 後のコミットが
複製を元マテリアルとして記録し、次の checkout が複製の複製を作る——という連鎖になります
（0.9.0-poc までの挙動。実アバターで552個・6段まで到達しました）。

不要になった複製は `Tools > AvatarVCS > Clean Up Generated Materials` で回収できます。
